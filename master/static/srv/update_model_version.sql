WITH mv AS (
  ...
  SET name = $2, comment = $3, readme = $4, last_updated_time = $6
  WHERE id = $1
  RETURNING id, version, checkpoint_uuid, model_id, last_updated_time, creation_time, name, comment, readme
),
m AS (
...
SELECT
    ...
    to_json(m) AS model,
    mv.version, mv.id, mv.creation_time, mv.name, mv.readme, mv.comment
    FROM c, m, mv;
