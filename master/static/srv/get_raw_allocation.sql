...
workloads AS (
    SELECT
        ...
        extract(epoch FROM
            ...
            upper(const.period * RANGE) - lower(const.period * RANGE)) AS seconds
    FROM (
        ...
        FROM (
            ...
            FROM (
                ...
                WHERE
                    ...
                    AND allocations.task_id = trials.task_id) metric_reports) derived_workload_spans
        WHERE
            start_time IS NOT NULL
            AND end_time IS NOT NULL
            AND kind IS NOT NULL) AS all_workloads,
        const
    WHERE
...
SELECT
    ...
    extract(epoch FROM
        -- `*` computes the intersection of the two ranges.
        upper(const.period * tstzrange(start_time, end_time)) - lower(const.period * tstzrange(start_time, end_time))) AS seconds
FROM
    agent_stats,
    const
WHERE
    const.period && tstzrange(start_time, end_time)
UNION
SELECT
    NULL AS experiment_id,
    'instance' AS kind,
