...
if [ "$DET_RESOURCES_TYPE" == "slurm-job" ]; then
    ...
    exec 1> >(tee -p >("$DET_PYTHON_EXECUTABLE" /run/determined/enrich_task_logs.py --stdtype stdout) >&1; : >$DET_LOG_WAIT_FIFO) \
         2> >(tee -p >("$DET_PYTHON_EXECUTABLE" /run/determined/enrich_task_logs.py --stdtype stderr) >&2; : >$DET_LOG_WAIT_FIFO)
