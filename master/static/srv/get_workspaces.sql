...
AND ($2 = ''
    OR w.user_id IN (
        SELECT
            unnest(string_to_array($2, ',')::int[])))
AND ($3 = ''
    OR w.name ILIKE $3)
AND ($4 = ''
    OR w.archived = $4::bool)
AND ($5 = ''
    OR (pins.id IS NOT NULL) = $5::bool)
GROUP BY
    w.id,
    u.username,
    pins.id
