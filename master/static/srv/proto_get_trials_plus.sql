...
trial_validations AS (
    SELECT
        ...
        v.end_time,
        v.metrics,
        ((v.metrics -> 'validation_metrics' ->> (searcher_info.metric_name))::float8 * searcher_info.sign) AS signed_searcher_metric
    FROM
        validations v
        INNER JOIN searcher_info ON v.trial_id = searcher_info.trial_id
    WHERE (v.metrics -> 'validation_metrics' ->> (searcher_info.metric_name)) IS NOT NULL
),
best_validation AS (
    SELECT
        v.trial_id,
    ...
    FROM (
        SELECT
            v.*,
            ROW_NUMBER() OVER (PARTITION BY v.trial_id ORDER BY v.signed_searcher_metric ASC) AS rank
        FROM
            trial_validations v) v
...
latest_validation AS (
    ...
    FROM (
        SELECT
            v.*,
            ROW_NUMBER() OVER (PARTITION BY v.trial_id ORDER BY v.end_time DESC) AS rank
        FROM
            trial_validations v) v
        JOIN searcher_info ON searcher_info.trial_id = v.trial_id
    WHERE
...
best_checkpoint AS (
    ...
    FROM (
        ...
        SELECT
            c.*,
            ROW_NUMBER() OVER (PARTITION BY c.trial_id ORDER BY c.signed_searcher_metric ASC) AS rank
        FROM (
            SELECT
                old_c.*
            FROM (
                SELECT
                    c.*,
                    v.signed_searcher_metric,
                    ROW_NUMBER() OVER (PARTITION BY v.trial_id ORDER BY v.signed_searcher_metric ASC) AS rank
            FROM
                trial_validations v
                INNER JOIN checkpoints_old_view c ON (c.steps_completed = v.total_batches
                        AND c.trial_id = v.trial_id)
            WHERE
                c.state = 'COMPLETED') old_c
        WHERE
            old_c.rank = 1
        UNION ALL
        SELECT
        ...
        FROM
            trial_validations v
            INNER JOIN checkpoints_new_view c ON (c.steps_completed = v.total_batches
                    AND c.trial_id = v.trial_id)
        WHERE
            c.state = 'COMPLETED') new_c
    WHERE
        new_c.rank = 1) c
    WHERE
        c.rank = 1) c
...
(
    SELECT
        extract(epoch FROM sum(coalesce(a.end_time, now()) - a.start_time))
    FROM
        allocations a
    WHERE
        a.task_id = t.task_id) AS wall_clock_time,
