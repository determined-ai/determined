UPDATE trials SET task_id = 'backported.' || id::text;
...
WITH today AS (
    SELECT date_trunc('day', current_timestamp AT TIME ZONE 'UTC') AS ts
),
     const AS (
         SELECT
             tstzrange(
                     today.ts,
                     (today.ts + interval '1 day')
                 ) AS period
         FROM today
     )
INSERT INTO allocations (
    SELECT
        ...
        coalesce(e.config #>> '{resources, resource_pool}', 'default') AS resource_pool,
