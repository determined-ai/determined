...
SET total_batches = COALESCE(
    (
        SELECT s.prior_batches_processed + s.num_batches AS total_batches
        FROM steps AS s
...
ADD CONSTRAINT checkpoints_trial_total_batches_unique UNIQUE (
    trial_id, total_batches
);
...
ADD CONSTRAINT validations_trial_total_batches_unique UNIQUE (
    trial_id, total_batches
);
...
ALTER TABLE steps
ADD CONSTRAINT steps_trial_total_batches_unique UNIQUE (
    trial_id, total_batches
);
