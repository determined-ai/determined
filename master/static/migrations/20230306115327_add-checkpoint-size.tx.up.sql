...
UPDATE
    public.checkpoints_v2
SET
    size = sub.size
FROM (
    SELECT
        id,
        coalesce(sum((size_tuple).value::text::bigint), 0) AS size
    FROM (
        SELECT
            jsonb_each(c.resources) AS size_tuple,
            id
        FROM
            public.checkpoints_v2 c
        WHERE
            c.resources != 'null'::jsonb) AS sizes
    GROUP BY
        id) AS sub
WHERE
    checkpoints_v2.id = sub.id;

ALTER TABLE public.raw_checkpoints
    ADD COLUMN size bigint NOT NULL DEFAULT 0;

UPDATE
    public.raw_checkpoints
SET
    size = sub.size
FROM (
    SELECT
    ...
    GROUP BY
        id) AS sub
WHERE
    public.raw_checkpoints.id = sub.id;
