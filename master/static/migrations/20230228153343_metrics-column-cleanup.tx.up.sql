...
DROP TYPE step_state;

DROP TYPE validation_state;
...
WITH b AS (
    SELECT
        trial_id,
        max(total_batches) total_batches
    FROM
        steps
    GROUP BY
        trial_id
...
ELSE
    -1.0 * (v.metrics -> 'validation_metrics' ->> (e.config -> 'searcher' ->> 'metric'))::float8
...
FROM
    ...
    LEFT JOIN validations v ON t.id = v.trial_id
        AND v.id = t.best_validation_id
    LEFT JOIN steps s ON t.id = s.trial_id
