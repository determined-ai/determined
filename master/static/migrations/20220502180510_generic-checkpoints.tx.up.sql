...
SELECT
    ...
    c.uuid AS uuid,
    t.task_id,
    CASE WHEN t.task_id IS NULL THEN
        NULL
    ELSE
...
FROM
    ...
    LEFT JOIN experiments AS e ON t.experiment_id = e.id
    LEFT JOIN validations AS v ON CAST(c.metadata ->> 'steps_completed' AS int) = v.total_batches
        AND t.id = v.trial_id
        -- avoiding the steps view causes Postgres to not "Materialize" in this join.
    LEFT JOIN raw_steps AS s ON CAST(c.metadata ->> 'steps_completed' AS int) = s.total_batches
        AND t.id = s.trial_id
WHERE
    s.archived IS NULL
    OR s.archived = FALSE;
...
SELECT
    ...
    jsonb_build_object('trial_id', c.trial_id, 'experiment_id', c.experiment_id, 'experiment_config', c.experiment_config, 'hparams', c.hparams,
        ...
        'training_metrics', jsonb_build_object('avg_metrics', c.training_metrics -> 'avg_metrics', 'batch_metrics', c.training_metrics -> 'batch_metrics'), 'validation_metrics', json_build_object('avg_metrics', c.validation_metrics), 'searcher_metric', c.searcher_metric) AS training
FROM
