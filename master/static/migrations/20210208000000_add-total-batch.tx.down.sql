...
ALTER TABLE checkpoints
    ADD COLUMN step_id integer NOT NULL DEFAULT 0;
...
SET
    step_id = COALESCE((
        ...
        FROM steps s
        WHERE
            c.total_batches = s.total_batches
            AND c.trial_id = s.trial_id), 0);

ALTER TABLE checkpoints
    ADD CONSTRAINT checkpoints_trial_step_unique UNIQUE (trial_id, step_id);

ALTER TABLE checkpoints
    DROP COLUMN total_batches;
...
SET
    step_id = COALESCE((
        ...
        FROM steps s
        WHERE
            v.total_batches = s.total_batches
            AND v.trial_id = s.trial_id), 0);
