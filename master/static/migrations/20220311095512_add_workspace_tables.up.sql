...
-- Indexes
CREATE INDEX ix_experiments_project_id ON public.experiments USING btree (
    project_id
);
CREATE INDEX ix_projects_workspace_id ON public.projects USING btree (
    workspace_id
);
CREATE INDEX ix_projects_user_id ON public.projects USING btree (user_id);
CREATE INDEX ix_project_models_project_id ON public.project_models USING btree (
    project_id
);
...
newp AS (
    INSERT INTO projects (
        name, description, archived, workspace_id, user_id, immutable
    )
    SELECT
        'Uncategorized',
        '',
