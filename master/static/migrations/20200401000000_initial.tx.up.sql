...
CREATE FUNCTION public.best_checkpoint_by_metric(
    tid integer, metric text, smaller_is_better boolean
...
CREATE FUNCTION public.experiments_best_validation_history(
    e public . experiments
) RETURNS SETOF public . validations
LANGUAGE sql STABLE
AS $$
    WITH is_best AS (
        SELECT v.id,
...
CREATE FUNCTION public.get_raw_metric(
    v public . validations, e public . experiments
...
CREATE FUNCTION public.get_signed_metric(
    v public . validations, e public . experiments
) RETURNS double precision
LANGUAGE sql STABLE
AS $$
...
ALTER TABLE ONLY public.agent_user_groups ALTER COLUMN id SET DEFAULT nextval(
    'public.agent_user_groups_id_seq'::regclass
);
...
ALTER TABLE ONLY public.config_files ALTER COLUMN id SET DEFAULT nextval(
    'public.config_files_id_seq'::regclass
);
...
ALTER TABLE ONLY public.user_sessions ALTER COLUMN id SET DEFAULT nextval(
...
ALTER TABLE ONLY public.users ALTER COLUMN id SET DEFAULT nextval(
...
CREATE INDEX ix_checkpoints_trial_id ON public.checkpoints USING btree (
    trial_id
);
...
CREATE INDEX ix_searcher_events_experiment_id ON public.searcher_events USING btree (
    experiment_id
);
...
CREATE INDEX ix_trials_experiment_id ON public.trials USING btree (
    experiment_id
);
...
CREATE INDEX ix_trials_warm_start_checkpoint_id ON public.trials USING btree (
...
CREATE INDEX ix_validations_trial_id ON public.validations USING btree (
    trial_id
);
...
ALTER TABLE ONLY public.agent_user_groups
ADD CONSTRAINT agent_user_groups_user_id_fkey FOREIGN KEY (
    user_id
...
ALTER TABLE ONLY public.checkpoints
ADD CONSTRAINT checkpoints_trial_id_fkey FOREIGN KEY (
    trial_id, step_id
) REFERENCES public.steps (trial_id, id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.experiments
ADD CONSTRAINT experiments_owner_fkey FOREIGN KEY (
    owner_id
) REFERENCES public.users (id);
...
ALTER TABLE ONLY public.searcher_events
ADD CONSTRAINT searcher_events_experiment_id_fkey FOREIGN KEY (
    experiment_id
...
ALTER TABLE ONLY public.steps
ADD CONSTRAINT steps_trial_id_fkey FOREIGN KEY (
    trial_id
...
ALTER TABLE ONLY public.trial_logs
ADD CONSTRAINT trial_logs_trial_id_fkey FOREIGN KEY (
    trial_id
) REFERENCES public.trials (id);
...
ALTER TABLE ONLY public.trials
ADD CONSTRAINT trials_experiment_id_fkey FOREIGN KEY (
    experiment_id
) REFERENCES public.experiments (id);
...
ALTER TABLE ONLY public.trials
ADD CONSTRAINT trials_warm_start_checkpoint_id_fkey FOREIGN KEY (
    warm_start_checkpoint_id
...
ALTER TABLE ONLY public.user_sessions
ADD CONSTRAINT user_sessions_user_id_fkey FOREIGN KEY (
    user_id
) REFERENCES public.users (id);
...
ALTER TABLE ONLY public.validations
ADD CONSTRAINT validations_trial_id_fkey FOREIGN KEY (
    trial_id, step_id
) REFERENCES public.steps (trial_id, id) ON DELETE CASCADE;
...
RESET search_path;
INSERT INTO users (username, admin, active) VALUES ('admin', 't', 't'),
('determined', 'f', 't');
