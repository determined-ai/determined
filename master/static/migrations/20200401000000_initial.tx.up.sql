...
--
CREATE FUNCTION public.best_checkpoint_by_metric (tid integer, metric text, smaller_is_better boolean)
    ...
    FROM
        checkpoints c
        JOIN validations v ON (c.trial_id,
            c.step_id) = (v.trial_id,
            v.step_id)
    WHERE
        c.trial_id = tid
    ...
    ORDER BY
        (
            SELECT
                CASE WHEN smaller_is_better THEN
                    1
                ELSE
                    -1
                END) * (v.metrics -> 'validation_metrics' ->> metric)::float8 ASC
        LIMIT 1
$$;
...
--
CREATE FUNCTION public.experiments_best_validation_history (e public.experiments)
    ...
    WITH is_best AS (
        SELECT
            v.id,
            coalesce(get_signed_metric (v, e) < min(get_signed_metric (v, e)) OVER (PARTITION BY e.id ORDER BY v.end_time ASC ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING), TRUE) AS is_best
        FROM
            trials t,
            validations v
        WHERE
            e.id = t.experiment_id
            AND t.id = v.trial_id
            AND v.state = 'COMPLETED'
)
    SELECT
...
--
CREATE FUNCTION public.frequencies (vals anyarray)
    RETURNS jsonb
    LANGUAGE sql
    IMMUTABLE
    AS $$
    SELECT
        coalesce(jsonb_agg(row_to_json(counts)), '[]'::jsonb)
    FROM (
        SELECT
            to_jsonb (unnest) AS value,
            count(*)
        FROM
            unnest(vals)
        GROUP BY
...
--
CREATE FUNCTION public.get_raw_metric (v public.validations, e public.experiments)
...
--
CREATE FUNCTION public.get_signed_metric (v public.validations, e public.experiments)
    RETURNS double precision
    LANGUAGE sql
    STABLE
    AS $$
    SELECT
        get_raw_metric (v, e) * (
            SELECT
                CASE WHEN coalesce((e.config -> 'searcher' ->> 'smaller_is_better')::boolean, TRUE) THEN
                    1
                ELSE
                    -1
                END)
...
--
CREATE FUNCTION public.try_float8_cast (text)
    RETURNS double precision
    LANGUAGE sql
    IMMUTABLE STRICT
    AS $_$
    SELECT
        CASE WHEN $1 ~ e'^-?(?:0|[1-9]\\d*)' '(?:\\.\\d+)?(?:[eE][+-]?\\d+)?$' THEN
            $1::float8
        END;
$_$;
...
ALTER TABLE public.checkpoints
    ...
    ADD GENERATED BY DEFAULT AS IDENTITY (SEQUENCE NAME
        public.checkpoints_id_seq START WITH 1 INCREMENT BY 1
        NO MINVALUE
        NO MAXVALUE
        CACHE 1);
...
ALTER TABLE public.experiments
    ALTER COLUMN id
    ADD GENERATED BY DEFAULT AS IDENTITY (SEQUENCE NAME
        public.experiments_id_seq START WITH 1 INCREMENT BY 1
        NO MINVALUE
        NO MAXVALUE
        CACHE 1);
...
--
ALTER TABLE public.searcher_events
    ALTER COLUMN id
    ADD GENERATED BY DEFAULT AS IDENTITY (SEQUENCE NAME
        public.searcher_events_id_seq START WITH 1 INCREMENT BY 1
        NO MINVALUE
        NO MAXVALUE
        CACHE 1);
...
ALTER TABLE public.trial_logs
    ALTER COLUMN id
    ADD GENERATED BY DEFAULT AS IDENTITY (SEQUENCE NAME
        public.trial_logs_id_seq START WITH 1 INCREMENT BY 1
        NO MINVALUE
        NO MAXVALUE
        CACHE 1);
...
ALTER TABLE public.trials
    ALTER COLUMN id
    ADD GENERATED BY DEFAULT AS IDENTITY (SEQUENCE NAME
        public.trials_id_seq START WITH 1 INCREMENT BY 1
        NO MINVALUE
        NO MAXVALUE
        CACHE 1);
...
ALTER TABLE public.validations
    ALTER COLUMN id
    ADD GENERATED BY DEFAULT AS IDENTITY (SEQUENCE NAME
        public.validations_id_seq START WITH 1 INCREMENT BY 1
        NO MINVALUE
        NO MAXVALUE
        CACHE 1);
...
ALTER TABLE ONLY public.agent_user_groups
    ALTER COLUMN id SET DEFAULT nextval('public.agent_user_groups_id_seq'::regclass);

--
-- Name: config_files id; Type: DEFAULT; Schema: public; Owner: -
--
ALTER TABLE ONLY public.config_files
    ALTER COLUMN id SET DEFAULT nextval('public.config_files_id_seq'::regclass);
...
INSERT INTO users (username, admin, active)
    VALUES ('admin', 't', 't'),
    ('determined', 'f', 't');
