...
func (l *taskList) GetAllocationByHandler(handler *actor.Ref) (*sproto.AllocateRequest, bool) {
...
func (l *taskList) AddTask(req *sproto.AllocateRequest) bool {
	if _, ok := l.GetAllocationByHandler(req.AllocationActor); ok {
		return false
	}

	l.taskByTime.Add(req)
	l.taskByHandler[req.AllocationActor] = req
	l.taskByID[req.AllocationID] = req
	return true
}

func (l *taskList) RemoveTaskByHandler(handler *actor.Ref) *sproto.AllocateRequest {
	req, ok := l.GetAllocationByHandler(handler)
	if !ok {
		return nil
	}
...
func registerTimeComparator(t1 *sproto.AllocateRequest, t2 *sproto.AllocateRequest) int {
	if !t1.AllocationActor.RegisteredTime().Equal(t2.AllocationActor.RegisteredTime()) {
		if t1.AllocationActor.RegisteredTime().Before(t2.AllocationActor.RegisteredTime()) {
			return -1
		}
		return 1
	}
