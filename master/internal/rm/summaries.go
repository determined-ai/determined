...
func getTaskSummary(
	...
	if req, ok := reqList.GetTaskByID(id); ok {
		summary := newTaskSummary(req, reqList.GetAllocations(req.AllocationRef), groups, schedulerType)
		return &summary
	}
	return nil
}
...
func getTaskSummaries(
	...
	for it := reqList.iterator(); it.next(); {
		...
		ret[req.AllocationID] = newTaskSummary(
			req, reqList.GetAllocations(req.AllocationRef), groups, schedulerType)
	}
	return ret
}
