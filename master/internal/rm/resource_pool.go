...
func (rp *ResourcePool) allocateRequest(ctx *actor.Context, msg sproto.AllocateRequest) {
	rp.notifyOnStop(ctx, msg.AllocationActor, sproto.ResourcesReleased{
	...
	if msg.Restore {
		...
		if err != nil {
			...
			ctx.Tell(msg.AllocationActor, rf)
...
func (rp *ResourcePool) restoreResources(
	...
	rp.taskList.SetAllocations(req.AllocationActor, &allocated)
	ctx.Tell(req.AllocationActor, allocated.Clone())
...
func (rp *ResourcePool) allocateResources(ctx *actor.Context, req *sproto.AllocateRequest) bool {
	...
	rp.taskList.SetAllocations(req.AllocationActor, &allocated)
	ctx.Tell(req.AllocationActor, allocated)
	...
	ctx.Log().Infof("allocated resources to %s", req.AllocationActor.Address())
...
func (c containerResources) Start(
	...
	return ctx.Ask(handler, sproto.StartTaskContainer{
		TaskActor: c.req.AllocationActor,
		StartContainer: aproto.StartContainer{
			Container: cproto.Container{
				Parent:  c.req.AllocationActor.Address(),
				ID:      c.containerID,
