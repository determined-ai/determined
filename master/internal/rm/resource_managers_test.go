...
// wipeSingletonRM wipes the singleton RM, for testing, since some tests may close it as they end.
func wipeSingletonRM(*testing.T) {
	once = sync.Once{}
...
func TestResourceManagerValidateRPResourcesUnknown(t *testing.T) {
	...
	New(system, nil, echo.New(), conf, nil, nil)
	defer wipeSingletonRM(t)

	resp, err := Get().ValidateCommandResources(system, sproto.ValidateCommandResourcesRequest{
