...
func (k KubernetesResourceManager) ResolveResourcePool(
	...
	return name, k.ValidateResourcePool(ctx, name)
}

// ValidateResourcePool validates a resource pool is none or the k8s dummy pool.
func (KubernetesResourceManager) ValidateResourcePool(ctx actor.Messenger, name string) error {
	if name != "" && name != KubernetesDummyResourcePool {
		return fmt.Errorf("invalid resource pool for k8s: %s", name)
	}
	return nil
}
...
func (k *kubernetesResourceManager) addTask(ctx *actor.Context, msg sproto.AllocateRequest) {
	actors.NotifyOnStop(ctx, msg.AllocationActor, sproto.ResourcesReleased{
		AllocationRef: msg.AllocationActor,
	})
	...
	ctx.Log().Infof(
		...
		msg.AllocationActor.Address(), msg.AllocationID,
	)
	if msg.IsUserVisible {
		...
		k.jobIDtoAddr[msg.JobID] = msg.AllocationActor
		k.addrToJobID[msg.AllocationActor] = msg.JobID
	...
	case sproto.SetGroupPriority:
		...
		for it := k.reqList.iterator(); it.next(); {
			if it.value().Group == msg.Handler {
				taskActor := it.value().AllocationActor
...
func (k *kubernetesResourceManager) assignResources(
	...
	for pod := 0; pod < numPods; pod++ {
		...
		rs := &k8sPodResources{
			...
			initialPosition: k.queuePositions[k.addrToJobID[req.AllocationActor]],
		}
		allocations[rs.Summary().ResourcesID] = rs
		k.addrToContainerID[req.AllocationActor] = containerID
		k.containerIDtoAddr[containerID.String()] = req.AllocationActor
	}

	assigned := sproto.ResourcesAllocated{ID: req.AllocationID, Resources: allocations}
	k.reqList.SetAllocationsRaw(req.AllocationActor, &assigned)
	req.AllocationActor.System().Tell(req.AllocationActor, assigned.Clone())

	ctx.Log().
		WithField("allocation-id", req.AllocationID).
		WithField("task-handler", req.AllocationActor.Address()).
...
func (k *kubernetesResourceManager) schedulePendingTasks(ctx *actor.Context) {
	for it := k.reqList.iterator(); it.next(); {
		req := it.value()
		group := k.groups[req.Group]
		assigned := k.reqList.GetAllocations(req.AllocationActor)
...
func (p k8sPodResources) Start(
	...
	return ctx.Ask(p.podsActor, kubernetes.StartTaskPod{
		TaskActor:  p.req.AllocationActor,
		Spec:       spec,
		Slots:      p.slots,
		Rank:       rri.AgentRank,
		LogContext: logCtx,
