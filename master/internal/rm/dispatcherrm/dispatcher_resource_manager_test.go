...
func Test_dispatcherResourceManager_checkLauncherVersion(t *testing.T) {
	...
	assert.Equal(t, checkMinimumLauncherVersion("3.2.3"), true)
...
// Verifies that the 'handleFailedJobTerminationRetries()' function keeps trying
// to call the job termination function until the number of retries have been to
// exhausted, and that it waits the specified 'X' number of seconds before
// calling the job termination function.
func Test_HandleFailedJobTerminationRetries(t *testing.T) {
	ctx := getMockActorCtx()

	dispatchID := "dispatchID1"
	allocationID := model.AllocationID("allocationID1")

	// Number of seconds to wait before calling the job termination function.
	delayBeforeCallingJobTerminationFunc := 1

	// Maximum number of times to call the job termination function before
	// giving up.
	maxRetryAttempts := 3

	// Keeps track of the number of times that the job termination function is
	// called.
	...
	jobTerminationFunc := func(
		...
		numTimesJobTerminationFuncCalled++
	}

	// Ensure the map is cleared.
	failedJobTerminations.Range(func(key interface{}, value interface{}) bool {
		failedJobTerminations.Delete(key)
		return true
	})

	/*
	 * Test the case where the dispatch ID should not be found when the map is
	 * empty.
	 */
	_, ok := failedJobTerminations.Load(dispatchID)

	assert.Equal(t,
		ok,
		false,
		fmt.Sprintf("Expected map to be empty, but dispatch ID '%s' was found",
			dispatchID))

	/*
	 * Test the case where we the first retry is performed.
	 */
	timeBefore := time.Now()

	performFailedJobTerminationRetries(ctx,
		dispatchID,
		allocationID,
		delayBeforeCallingJobTerminationFunc,
		maxRetryAttempts,
		jobTerminationFunc)
	...
	// While the retries have not been exhausted, the dispatch ID should remain
	// in the map.
	assert.Equal(t,
		ok,
		true,
		fmt.Sprintf("Expected dispatch ID '%s' to be found",
			dispatchID))

	// Verify that 1 retry has been performed.
	assert.Equal(t,
		val.(int),
		1,
		fmt.Sprintf("Unexpected value found for dispatch ID '%s'",
			dispatchID))

	// Verify that the job termination function was called for the first time.
	assert.Equal(t,
		numTimesJobTerminationFuncCalled,
		1,
		"Expected jobTerminationFunc to be called 1 time")

	// There should have been a delay before calling the job termination function.
	assert.Equal(t,
		elapsedTime >= time.Duration(delayBeforeCallingJobTerminationFunc)*time.Second,
		true,
		"Did not wait the expected time before calling jobTerminationFunc")

	/*
	 * Test the case where we the second retry is performed.
	 */
	timeBefore = time.Now()
	...
	// Verify that 2 retries have been performed.
	assert.Equal(t,
		val.(int),
		2,
		fmt.Sprintf("Unexpected value found for dispatch ID '%s'",
	...
	// Verify that the job termination function was called a second time.
	assert.Equal(t,
		numTimesJobTerminationFuncCalled,
		2,
		"Expected jobTerminationFunc to be called 2 times")
	...
	/*
	 * Test the case where we the third retry is performed.
	 */
	timeBefore = time.Now()
	...
	// Verify that 3 retries have been performed.
	assert.Equal(t,
		val.(int),
		3,
		fmt.Sprintf("Unexpected value found for dispatch ID '%s'",
	...
	// Verify that the job termination function was called a third time.
	assert.Equal(t,
		numTimesJobTerminationFuncCalled,
		3,
		"Expected jobTerminationFunc to be called 3 times")
	...
	/*
	 * Test the case where all retries have been exhausted.
	 */
	timeBefore = time.Now()
	...
	// After all retries have been exhausted, the dispatch ID should have been
	// automatically removed from the map.
	assert.Equal(t,
		ok,
		false,
		fmt.Sprintf("Expected for dispatch ID '%s' to be removed from map, but was still found",
			dispatchID))

	// The jobTerminationFunc should not have been called again, so verify that
	// it was not called a fourth time. That is, the count should remain at 3.
	assert.Equal(t,
		numTimesJobTerminationFuncCalled,
		3,
		"Expected jobTerminationFunc to be called 3 times")

	// When all retries are exhausted, there should not be any delay, because
	// we should no longer be calling the job termination function.
	assert.Equal(t,
		elapsedTime < time.Duration(delayBeforeCallingJobTerminationFunc)*time.Second,
		true,
		"Did not expect to wait since jobTerminationFunc should not have been called")
}
