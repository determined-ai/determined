...
const (
	...
	jobLogFormat = "Skipping foreign dispatch, ID %s, name %s"
)
...
// isForeignJob returns true if the HPC job is not using one of the names
...
func gcTerminateRunningOrphans(
	...
	for _, v := range dispatches {
		...
		if isForeignJob(v) {
			log.Debugf(jobLogFormat, dispatchID, *v.LaunchedCapsuleReference.Name)
			continue
		}
