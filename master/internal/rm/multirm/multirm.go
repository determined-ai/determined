...
func New(
	...
	// doesn't exist.
	defaultRMName := config.DefaultRMName
...
func (m *MultiRMRouter) getRM(rmName string) string {
	if rmName == "" {
		m.syslog.Infof("RM undefined, routing to default manager")
		return m.defaultRMName
	}

	_, ok := m.rms[rmName]
	if !ok {
		m.syslog.WithError(ErrRMNotDefined(rmName))
		return m.defaultRMName
	}
	return rmName
}
