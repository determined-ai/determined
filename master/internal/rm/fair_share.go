...
func fairshareSchedule(
	...
	for it := taskList.iterator(); it.next(); {
		req := it.value()
		allocations := taskList.GetAllocations(req.AllocationActor)
	...
	for _, group := range states {
		for _, state := range group {
			...
			for _, req := range state.reqs {
				allocated := taskList.GetAllocations(req.AllocationActor)
	...
	for _, state := range states {
		if state.activeSlots > state.offered {
			...
			for _, req := range state.allocatedReqs {
				if req.Preemptible {
					toRelease = append(toRelease, req.AllocationActor)
					state.activeSlots -= req.SlotsNeeded
					if state.activeSlots <= state.offered {
						break
					}
