...
func (r *ActorResourceManager) ask(ctx actor.Messenger, req interface{}, v interface{}) error {
	if reflect.ValueOf(v).IsValid() && !reflect.ValueOf(v).Elem().CanSet() {
		return status.Errorf(
			codes.Internal,
			"ask to actor %s contains valid but unsettable response holder %T", r.ref.Address(), v,
		)
	}
	...
	case resp.Source() == nil:
		return status.Errorf(
			codes.NotFound,
			"actor %s could not be found", r.ref.Address(),
		)
	case expectingResponse && resp.Empty(), expectingResponse && resp.Get() == nil:
		return status.Errorf(
			codes.NotFound,
			"actor %s did not response", r.ref.Address(),
		)
	case resp.Error() != nil:
	...
	default:
		if expectingResponse {
			if reflect.ValueOf(v).Elem().Type() != reflect.ValueOf(resp.Get()).Type() {
				return status.Errorf(
					codes.Internal,
					"actor %s returned unexpected message (%T): %v", r.ref.Address(), resp, resp,
				)
			}
