...
func (a *apiServer) BulkAutoCreateWorkspaceNamespaceBindings(ctx context.Context,
	...
	wkspBinding := map[int][]string{}
	for _, v := range wsns {
		if _, ok := wkspBinding[v.WorkspaceID]; !ok {
			wkspBinding[v.WorkspaceID] = []string{v.ClusterName}
		} else {
			wkspBinding[v.WorkspaceID] = append(wkspBinding[v.WorkspaceID], v.ClusterName)
		}
	}

	if err = a.bulkAutoCreateWorkspaceNamespaceBindingsHelper(ctx, allWorkspaceIds, wkspBinding); err != nil {
		return nil, err
	}
...
func (a *apiServer) bulkAutoCreateWorkspaceNamespaceBindingsHelper(
	ctx context.Context, wkspIDList []int, wkspBinding map[int][]string,
) error {
