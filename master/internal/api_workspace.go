...
func generateNamespaceName(clusterID string, workspace string, wkspID int) string {
	...
	namespace := workspacePrefix + "-" + detNamespacePrefix + "-" + clusterIDPrefix + "-" +
		workspaceID
	return namespace
}
...
func (a *apiServer) SetResourceQuotas(ctx context.Context,
	req *apiv1.SetResourceQuotasRequest) (*apiv1.SetResourceQuotasResponse, error) {
...
func getAutoGeneratedNamespaceName(ctx context.Context, wkspID int, tx *bun.Tx) (*string, error) {
	...
	ns := generateNamespaceName(clusterID, wsToDelete.Name, wsToDelete.ID)
	namespace = ns
