...
// workspaces).
func generateNamespaceName(detInstallationNamespace, clusterID, workspaceName string, wkspID uint) string {
...
func getAutoGeneratedNamespaceName(ctx context.Context, wkspID int, tx *bun.Tx) (*string, error) {
	...
	ns := generateNamespaceName(installationNamespace, clusterID, wsToDelete.Name, uint(wsToDelete.ID))
	namespace = ns
