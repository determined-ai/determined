...
func (a *apiServer) setWorkspaceNamespaceBindings(ctx context.Context,
	...
	autoCreatedNamespace, err := getAutoCreatedNamespace(ctx, w, tx)
	if err != nil {
		return nil, err
	}
...
func (a *apiServer) setResourceQuotas(ctx context.Context,
	...
	autoGeneratedNamespace, err := getAutoCreatedNamespace(ctx, w, tx)
	if err != nil {
		fmt.Println("This is the error! :%w \n \n ", err)
		return nil, fmt.Errorf("error getting generated namespace name for workspace %s: %w",
			w.Name, err)
	}
...
func (a *apiServer) DeleteWorkspace(
	...
	workspace, _, err := a.getWorkspaceAndCheckCanDoActions(ctx, req.Id, false,
	...
	err = db.Bun().RunInTx(
		...
		func(ctx context.Context, tx bun.Tx) error {
			...
			autoGeneratedNamespaceName, err := getAutoCreatedNamespace(ctx, workspace, &tx)
