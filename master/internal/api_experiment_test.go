...
func TestExperimentSearchApiFilterParsing(t *testing.T) {
	...
	validTestCases := [][2]string{
		...
		{`(experiment.description~"experiment description" AND (-experiment.id:456 OR -experiment.resourcePool~"test\"s comma value\"s"))`, `(e.config->>'description' LIKE '%experiment description%' AND (e.id != 456 OR e.config->'resources'->>'resource_pool' NOT LIKE '%test\"s comma value\"s%'))`},
		{`(experiment.forkedFrom:5 OR (-validation.error:1 AND hp.hyperparameter<=10))`, `(e.parent_id = 5 OR ((besttrials.best_validation->'metrics'->'avg_metrics'->>'error')::float8 != 1 AND (e.config->'hyperparameters'->'hyperparameter'->>'val')::float8<=10))`},
	}
