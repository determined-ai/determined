...
func TestExperimentSearchApiFilterParsing(t *testing.T) {
	...
	validTestCases := [][2]string{
		...
		{"validation.validation_accuracy>1", "besttrials.best_validation->'metrics'->'avg_metrics'->>'validation_accuracy'>1"},
		...
		{`(general_column.forkedFrom:5 OR (-validation.error:1 AND hp.hyperparameter<=10))`, `(e.parent_id = 5 OR (besttrials.best_validation->'metrics'->'avg_metrics'->>'error' != 1 AND e.config->'hyperparameters'->>'hyperparameter'<=10))`},
	}
