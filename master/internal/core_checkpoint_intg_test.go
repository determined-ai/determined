...
const (
	S3_TEST_BUCKET = "storage-intg-tests"
...
func createCheckpointS3(bucket string, prefix string) error {
...
func checkTgz(t *testing.T, content io.Reader, id string) {
	...
	for {
		...
		fmt.Printf("Fiel in tar: %s, size: %d, type: %c\n", hdr.Name, hdr.Size, hdr.Typeflag)
...
func createCheckpointDB(t *testing.T, pgDB *db.PgDB, id uuid.UUID) {
...
func createCheckpoint(t *testing.T, pgDB *db.PgDB) (string, error) {
	id := uuid.New()
	createCheckpointDB(t, pgDB, id)
	err := createCheckpointS3(S3_TEST_BUCKET, S3_TEST_PREFIX+"/"+id.String())
...
func TestAuthZGetCheckpointEchoSucceeded(t *testing.T) {
	var id string
	cases := []struct {
		DenyFuncName string
		...
		{"CanGetCheckpointTgz", func(id string) error {
			api, _, _ := SetupCheckpointAuthTestEcho(t)
	...
	for _, curCase := range cases {
		require.Equal(t, nil, curCase.IDToReqCall(id))
	}
}

func TestAuthZGetCheckpointEchoFailed(t *testing.T) {
