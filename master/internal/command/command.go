...
func (c *command) setPriority(ctx *actor.Context, priority int, forward bool) error {
	// TODO(Brad): Pack this into the interface.
	...
	if forward {
		if err := c.rm.SetGroupPriority(ctx, sproto.SetGroupPriority{
			Priority: priority,
			Handler:  ctx.Self(),
		}); err != nil {
...
func (c *command) setWeight(ctx *actor.Context, weight float64) error {
	// TODO(Brad): Pack this into interface.
	if sproto.UseK8sRM(ctx.Self().System()) {
	...
	if err := c.rm.SetGroupWeight(ctx, sproto.SetGroupWeight{
		Weight:  weight,
		Handler: ctx.Self(),
	}); err != nil {
