...
// manually or automatically by AWS, the spot request will enter a terminal state (either canceled,
// closed or disabled).
//
...
// without user interaction, but the user should be made aware of them because the user may be able to
// intervene and solve the problem. It is not clear how to differentiate these cases, so we handle them
// identically.
//
...
func (c *awsCluster) listSpot(ctx *actor.Context) ([]*Instance, error) {
	...
	// or they could have entered an inactive state for some reason.
	...
	canceledButInstanceRunningReqs, err := c.listCanceledButInstanceRunningSpotInstanceRequests(ctx, false)
	if err != nil {
		return nil, errors.Wrap(err, "cannot describe EC2 spot requests")
	}
...
func (c *awsCluster) terminateSpot(ctx *actor.Context, instanceIDs []*string) {
	...
	terminateInstancesResponse, err := c.terminateInstances(instancesToTerminate.asListOfPointers(), false)
	if err != nil {
...
// Convert c.spot.trackedReqs to a list of Instances. For the requests that have been fulfilled, this
// requires querying the EC2 API to find the instance state
...
func (c *awsCluster) createSpotInstanceRequestsCorrectingForClockSkew(
	...
	for numRetries := 0; numRetries <= maxRetries; numRetries++ {
		...
		if awsErr, ok := err.(awserr.Error); ok {
			ctx.Log().Infof("AWS error while launching spot instances, %s, %s", awsErr.Code(), awsErr.Message())
