...
// New wraps the given, underlying *websocket.Conn and returns a higher level, thread-safe wrapper.
func New[TIn, TOut any](name string, conn *websocket.Conn) *Websocket[TIn, TOut] {
...
func (s *Websocket[TIn, TOut]) runReadLoop(ctx context.Context, inbox chan<- TIn) error {
	...
	s.conn.SetPongHandler(func(string) error {
		if err := s.conn.SetReadDeadline(time.Now().Add(pongWait)); err != nil {
			s.logger.WithError(err).Error("setting read deadline")
		}
		return nil
	})
