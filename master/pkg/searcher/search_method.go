...
type SearchMethod interface {
	...
	// completing.
	validationCompleted(ctx context, requestID model.RequestID, metric float64, op ValidateAfter) ([]Operation, error)
...
func (defaultSearchMethod) validationCompleted(
	context, model.RequestID, float64, ValidateAfter,
) ([]Operation, error) {
	return nil, nil
}
