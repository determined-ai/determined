...
func (p *OperationPlanner) Plan(ops []Operation) (plannedOps []Operation) {
	for _, op := range ops {
		...
		case Train:
			...
			p.sequenceSearcherOp(tOp.RequestID, tOp, RunStep)
			plannedOps = append(plannedOps, workloads...)
		case Validate:
			workloads := p.validate(tOp.RequestID)
			p.sequenceSearcherOp(tOp.RequestID, tOp, ComputeValidationMetrics)
			plannedOps = append(plannedOps, workloads...)
		case Checkpoint:
			workloads := p.checkpoint(tOp.RequestID)
			p.sequenceSearcherOp(tOp.RequestID, tOp, CheckpointModel)
			plannedOps = append(plannedOps, workloads...)
		default:
			plannedOps = append(plannedOps, tOp)
		}
...
func (p *OperationPlanner) trainStep(requestID RequestID, numBatches int) (ops []Operation) {
	...
	ops = append(ops, NewTrainWorkload(requestID, p.stepCounts[requestID], numBatches))
	p.sequenceWorkloads(requestID, ops)
	return ops
}
...
func (p *OperationPlanner) checkpointByStepID(requestID RequestID, stepID int) (ops []Operation) {
	ops = append(ops, NewCheckpointWorkload(requestID, stepID))
	p.sequenceWorkloads(requestID, ops)
	return ops
}
...
func (p *OperationPlanner) sequenceWorkloads(requestID RequestID, ops []Operation) {
...
func (p *OperationPlanner) sequenceSearcherOp(
	requestID RequestID, searcherOp Operation, completedByLast Kind,
) {
...
func (p *OperationPlanner) WorkloadCompleted(
	...
	if workload.Kind == ComputeValidationMetrics {
		...
		case model.AllCheckpointPolicy:
			checkpoints = p.checkpointByStepID(requestID, workload.StepID)
