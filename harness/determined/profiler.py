...
class Timing:
    ...
    def to_measurement(self, accumulate: bool = False) -> NamedMeasurement:
        ...
        self.start_time = cast(float, self.start_time)
        start_time_dt = datetime.datetime.fromtimestamp(self.start_time, datetime.timezone.utc)
...
class MetricBatch:
    ...
    def convert_to_timestamp_str(timestamp: datetime.datetime) -> str:
        ...
        utcoffset = cast(datetime.timedelta, timestamp.utcoffset())
        assert utcoffset.total_seconds() == 0, (
            f"All datetime.datetime objects to be serialized must be in UTC, but the utcoffset was "
...
class ThroughputTracker:
    ...
    def add(self, new_val: float, batch_idx: int) -> Measurement:
        ...
        """
        now = time.time()
        timestamp = datetime.datetime.fromtimestamp(now, datetime.timezone.utc)
