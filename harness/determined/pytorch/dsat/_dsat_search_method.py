...
class BaseDSATSearchMethod(searcher.SearchMethod):
    ...
    def __init__(self, args: argparse.Namespace, exp_config: Dict[str, Any]) -> None:
        ...
        self.search_range_factor = args.search_range_factor
    ...
    def round_mbs_down(self, mbs: int) -> int:
        ...
        conditions specified by args.
        """
        if self.trial_tracker.train_batch_size is None:
            factor = self.trial_tracker.divisible_by
            mbs = factor * (mbs // factor)
        else:
            factor = self.trial_tracker.train_batch_size // self.trial_tracker.slots_per_trial
            while factor % mbs:
                mbs -= 1
        return mbs

    def round_mbs_up(self, mbs: int) -> int:
        """
...
class RandomDSATSearchMethod(BaseDSATSearchMethod):
    ...
    def get_trial_list_after_successful_run(
        ...
        if last_trial.mbs == self.trial_tracker.max_mbs:
            ...
            while new_search_data.lo > new_search_data.hi:
                ...
                new_search_data.hi = self.round_mbs_up(new_search_data.hi)
    ...
    def get_random_hparams_and_search_data(
        ...
        if best_trial_for_stage is not None and best_trial_for_stage.search_data is not None:
            ...
            # Update the floor to match the best trial's mbs used and use the  best trial's ceiling.
            new_search_data.lo = best_trial_for_stage.mbs
            new_search_data.hi = best_trial_for_stage.search_data.hi
        # Otherwise choose the corresponding search data based on approximate computations
        else:
            hi, lo = self.get_default_hi_lo(zero_stage)
