...
# their own dataloader, it will most likely be one created as part of DeepSpeed model engine
...
class DeepSpeedTrialController(det.TrialController):
    ...
    def pre_execute_hook(
        ...
        manual_dist_init = os.environ.get("MANUAL_INIT_DISTRIBUTED")
    ...
    def _train_for_step(
        ...
        for batch_idx in range(start, end):
            ...
            if not isinstance(model0, deepspeed.PipelineEngine):
                assert model0.micro_steps % self.context.num_micro_batches_per_slot == 0
...
class DeepSpeedTrial(det.Trial):
    ...
    * **Run forward and backward passes**.
    ...
    def __init__(self, context: det_ds.DeepSpeedTrialContext) -> None:
        ...
        #. If desired, use a custom model parallel unit by calling ``context.set_mpu``.
        #. If desired, disable automatic gradient accumulation by calling
           ``context.disable_auto_grad_accumulation``.
        #. If desired, use a custom dataloader by calling
