...
class DeepSpeedTrialContext(det.TrialContext, pytorch._PyTorchReducerContext):
    ...
    4. Disable data reproducibility checks to allow custom dataloaders.
    ...
    def set_mpu(self, mpu: det_ds.ModelParallelUnit) -> None:
        ...
        on data parallel topology and whether a rank should compute metrics/build dataloaders.

        This should only be needed if training with custom model parallelism.

        In the case of multiple model parallel engines, we assume that the MPU and dataloaders
