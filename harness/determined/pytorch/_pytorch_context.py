...
class PyTorchTrialContext(pytorch._PyTorchReducerContext):
    ...
    def _maybe_reset_tbd_writer(self) -> None:
        """
        Reset the TensorBoard writer (close so the next write will reopen a new file) if more than
        1 second has elapsed since the last reset. This is to ensure writes < 1 second apart do
        not overwrite the same file name generated by TensorBoard from the epoch second.
        """
        if self._tbd_writer is None:
            return
