...
class PTLAdapter(PyTorchTrial):
    ...
    # QUESTION: take uninstantiated lightning and datamodule so we isntatiate it instead? less code
    # for the user but might be better if the user sees this?
    def __init__(self, context: PyTorchTrialContext, lightning_module: ptl.LightningModule):
        ...
        (or lr_dict).
        - Dictionary, with an ‘optimizer’ key, and (optionally) a ‘lr_scheduler’ key whose value is
        a single LR scheduler or lr_dict.
