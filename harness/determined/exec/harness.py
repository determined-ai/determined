...
from determined import gpu, horovod, layers, load, workload, profiler
    ...
    with layers.SocketManager(env) as socket_mgr:
        ...
        with maybe_load_checkpoint(storage_mgr, env.latest_checkpoint) as load_path:
            ...
            if hvd_config.use:
                ...
                prof_start_on_batch, prof_end_after_batch = env.experiment_config.profiling_interval()
                with profiler.ProfilerAgent(trial_id=env.det_trial_id,
