...
class EnvironmentImageV0(schemas.SchemaBase):
    ...
    def runtime_defaults(self) -> None:
        if self.cpu is None:
            self.cpu = "determinedai/environments:py-3.8-pytorch-1.12-tf-2.8-cpu-ae93cdb"
        if self.rocm is None:
            self.rocm = "determinedai/environments:rocm-5.0-pytorch-1.10-tf-2.7-rocm-ae93cdb"

        if self.cuda is None:
            self.cuda = "determinedai/environments:cuda-11.3-pytorch-1.12-tf-2.8-gpu-ae93cdb"
