...
class BatchMetricWriter:
    ...
    def on_train_step_end(
        ...
        for batch_idx, batch_metrics in enumerate(metrics["batch_metrics"]):
            batches_seen = total_batches - len(metrics["batch_metrics"]) + batch_idx
        ...
        for name, value in metrics["avg_metrics"].items():
            if name in metrics_seen:
                continue
            self._maybe_write_metric(name, value, total_batches)

        self.writer.reset()

    def on_validation_step_end(self, total_batches: int, metrics: Dict[str, Any]) -> None:
