...
def list_users(args: Namespace) -> None:
    ...
    if resp.to_json().get("rbacEnabled"):
        render.render_objects(
            FullUserNoAdmin, client.list_users(include_inactive=args.include_inactive)
        )
    else:
        render.render_objects(FullUser, client.list_users(include_inactive=args.include_inactive))
    ...
    Cmd("u|ser", None, "manage users", [
        Cmd("list ls", list_users, "list users", [
            Arg(
                "--include-inactive",
                action="store_true",
                help="include both active and inactive users",
            )], is_default=True),
