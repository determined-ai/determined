...
def assert_events_match(events: Events, *patterns: str) -> List[Any]:
    """
    Make sure the events logged by our callback match the patterns specified by the test.
    ...
    Returns the data field of the events corresponding to the positive patterns.
    """
    matched_data = []

    def iter_patterns() -> Iterator[Tuple[List[re.Pattern], Optional[re.Pattern]]]:
        negatives = []
    ...
    for negatives, positive in iter_patterns():
...
def test_assert_events_match() -> None:
    ...
    def expect_success(events: Events, *patterns: str) -> None:
        try:
            assert_events_match(events, *patterns)
        except AssertionError:
            raise AssertionError(f"expected success: {patterns}")

    def expect_failure(events: Events, *patterns: str) -> None:
        try:
            assert_events_match(events, *patterns)
        except AssertionError:
            pass
...
def mock_core_context(
    path: str, events: Events, distributed: Optional[core.DistributedContext] = None
...
class DeterminedCallbackForTesting(det.keras.DeterminedCallback):
    ...
    def __init__(self, events: Events, *args: Any, **kwargs: Any) -> None:
        self.events = events
...
def test_basic_logic(tmp_path: pathlib.Path) -> None:
    ...
    check_keras_metrics(data[3])
    check_keras_metrics(data[5])
