...
class XORTrial(keras.TFKerasTrial):
    ...
    def build_training_data_loader(self) -> keras.InputData:
        train, _ = utils.make_xor_data_sequences(batch_size=4)
        return train

    def build_validation_data_loader(self) -> keras.InputData:
        _, test = utils.make_xor_data_sequences(batch_size=4)
        return test
...
class CustomOptimizer(optimizers.SGD):  # type: ignore
    pass
    ...
    def build_model(self) -> models.Sequential:
        ...
        model.compile(
            self.context.wrap_optimizer(
                CustomOptimizer(learning_rate=self.context.get_hparam("learning_rate"))
            ),
