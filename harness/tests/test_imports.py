...
def test_import_from_path() -> None:
    ...
    assert not os.path.exists(fx / "b" / "__pycache__")
    assert not os.path.exists(fx / "c" / "__pycache__")

    # We will have created some __pycache__ dirs but we don't want to pollute.
    shutil.rmtree(fx / "a" / "__pycache__")
    shutil.rmtree(fx / "lib" / "__pycache__")
