...
class DebugConfig:
    def __init__(
        ...
        if 0.0 < self._resource_profile_period_sec < 0.1:
            logging.warning(
                "resource profiling with psutil and a resource_profile_period_sec less than .1 "
                "seconds is unlikely to yield accurate results, adjusting to .1 seconds."
            )
            self._resource_profile_period_sec = 0.1
