...
class PiecewiseConstantDecayWithWarmup(tf.keras.optimizers.schedules.LearningRateSchedule):
    ...
    def _get_learning_rate(self, step):
        ...
        with tf.compat.v1.name_scope(
            ...
            return tf.cond(
                step < self.warmup_steps, lambda: warmup_lr(step), lambda: piecewise_lr(step)
            )
...
def set_gpu_thread_mode_and_count(flags_obj):
    ...
    if not flags_obj.datasets_num_private_threads:
        flags_obj.datasets_num_private_threads = min(
            cpu_count - total_gpu_thread_count - num_runtime_threads, flags_obj.num_gpus * 8
        )
        tf.compat.v1.logging.info(
            "Set datasets_num_private_threads to %s", flags_obj.datasets_num_private_threads
        )
...
def get_callbacks(learning_rate_schedule_fn=None, num_images=None):
    ...
    if not FLAGS.use_tensor_lr and learning_rate_schedule_fn:
        lr_callback = LearningRateBatchScheduler(
            learning_rate_schedule_fn, batch_size=FLAGS.batch_size, num_images=num_images
        )
...
def define_keras_flags(dynamic_loss_scale=True):
    ...
    flags.DEFINE_boolean(
        name="use_trivial_model", default=False, help="Whether to use a trivial Keras model."
    )
    flags.DEFINE_boolean(
    ...
    flags.DEFINE_boolean(
        name="use_tensor_lr", default=False, help="Use learning rate tensor instead of a callback."
    )
    flags.DEFINE_boolean(
        name="enable_tensorboard", default=False, help="Whether to enable Tensorboard callback."
    )
...
def get_synth_input_fn(
    height, width, num_channels, num_classes, dtype=tf.float32, drop_remainder=True
):
  ...
  """
    ...
    def input_fn(is_training, data_dir, batch_size, *args, **kwargs):
        ...
        inputs = tf.random.truncated_normal(
            [height, width, num_channels], dtype=dtype, mean=127, stddev=60, name="synthetic_inputs"
        )
...
def _monkey_patch_org_create_device_dataset():
    ...
    org_create_device_dataset_code = inspect.getsource(
        multi_device_iterator_ops.MultiDeviceIterator._create_device_dataset
    ...
    code_lines.insert(
        2, "    from tensorflow.python.data.experimental.ops import sleep as sleep_ops"
    )  # pylint: disable=line-too-long
    patched_code = "\n".join(line[2:] for line in code_lines)
    cls_multi_device_iterator.body[0].body[2] = ast.parse(patched_code).body[0]
    exec(
        compile(cls_multi_device_iterator, "<string>", "exec"),  # pylint: disable=exec-used
