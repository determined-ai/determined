...
def fpn_map_rois_to_levels(boxes):
    ...
    level = tf.cast(
        tf.floor(4 + tf.log(sqrtarea * (1.0 / 224) + 1e-6) * (1.0 / np.log(2))), tf.int32
    )
    ...
    level_ids = [
        tf.reshape(x, [-1], name="roi_level{}_id".format(i + 2)) for i, x in enumerate(level_ids)
    ]
    num_in_levels = [
        tf.size(x, name="num_roi_level{}".format(i + 2)) for i, x in enumerate(level_ids)
    ]
...
def generate_fpn_proposals(multilevel_pred_boxes, multilevel_label_logits, image_shape2d):
    ...
    if cfg.FPN.PROPOSAL_MODE == "Level":
        fpn_nms_topk = (
            cfg.RPN.TRAIN_PER_LEVEL_NMS_TOPK if training else cfg.RPN.TEST_PER_LEVEL_NMS_TOPK
        )
        for lvl in range(num_lvl):
