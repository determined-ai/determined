...
class ContrastiveLearningTrial(det_torch.PyTorchTrial):
    def __init__(self, context: det_torch.PyTorchTrialContext) -> None:
        ...
        # We are going to create a separate dataloader for training the classifier.
        # With dtrain, calling get_data_loader on the Determined dataloader will automatically
        # shard the dataset.
        self.train_cls_dataloader = self.build_train_cls_data_loader().get_data_loader(
            repeat=False,
    ...
    def build_train_cls_data_loader(self) -> Any:
        assert (
            self.data_config.dataset in STATS
        ), "only supports cifar10 and cifar100 datasets"
        mean = STATS[self.data_config.dataset]["mean"]
    ...
    def train_batch(
        ...
        if (
            ...
            and (epoch_idx + 1) % self.hparams.train_cls_every_n_epochs == 0
        ):
            print(f"Training classifier for {self.hparams.train_cls_epochs} epochs...")
            for e in range(self.hparams.train_cls_epochs):
                print(f"  training {e} epoch")
                for i, batch in enumerate(self.train_cls_dataloader):
                    images, labels = batch
                    images = images.cuda()
                    labels = labels.cuda()
