...
class EffDetTrial(PyTorchTrial):
    def __init__(self, context: PyTorchTrialContext) -> None:
        ...
        if self.args.model_ema:
            ...
            if self.args.sync_bn:
                ...
                self.model_ema = ModelEma(self.model_ema, context=self.context,decay=self.args.model_ema_decay)
            else:
    ...
    def build_training_data_loader(self):
        ...
        if not self.context.get_hparam("fake_data") and self.model_config.num_classes < loader_train.dataset.parser.max_label:
            logging.error(
        ...
        if reduced_loss is np.nan or math.isnan(reduced_loss):
            ...
            if not self.context.get_hparam("fake_data"):
                for name, p in self.model.named_parameters():
                    print('Nan occurred: ',name,'norm: ', p.grad.norm().item(), 'sum: ', p.grad.sum().item(), 'max: ', p.grad.min().item(), 'min: ', p.grad.min().item())
            reduced_loss = 0
