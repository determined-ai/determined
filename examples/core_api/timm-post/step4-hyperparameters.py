...
parser.add_argument(
    "--fuser", default="", type=str, help="Select jit fuser. One of ('', 'te', 'old', 'nvfuser')"
)
parser.add_argument(
    "--grad-checkpointing",
    action="store_true",
...
parser.add_argument(
    "--layer-decay", type=float, default=None, help="layer-wise learning rate decay (default: None)"
)

# Learning rate schedule parameters
parser.add_argument(
    "--sched", default="cosine", type=str, metavar="SCHEDULER", help='LR scheduler (default: "step"'
)
parser.add_argument(
...
parser.add_argument(
    "--decay-epochs", type=float, default=100, metavar="N", help="epoch interval to decay LR"
)
parser.add_argument(
    "--warmup-epochs",
    type=int,
...
parser.add_argument(
    "--cutmix", type=float, default=0.0, help="cutmix alpha, cutmix enabled if > 0. (default: 0.)"
)
parser.add_argument(
    "--cutmix-minmax",
    type=float,
...
parser.add_argument(
    "--drop-path", type=float, default=None, metavar="PCT", help="Drop path rate (default: None)"
)
parser.add_argument(
...
class DeterminedCheckpointSaver(CheckpointSaver):
    def __init__(self, core_context, trial_id, epoch_length, *args, **kwargs):
...
def main(core_context):
    ...
    resume_epoch = None
    # Determined checkpoint file lifetimes are limited by the restore_path context manager.
    # We use nullcontext to stand in when there's no Determined checkpoint.
    ...
    with restore_path_context as restore_path:
        # Determined resumption overrides the command line --resume.
    ...
    # FileLock solves an occasional race condition when downloading datasets.
    os.makedirs(args.data_dir, exist_ok=True)
    with filelock.FileLock(os.path.join(args.data_dir, "lock")):
