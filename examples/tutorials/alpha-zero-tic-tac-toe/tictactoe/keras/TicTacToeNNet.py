...
class TicTacToeNNet:
    def __init__(self, game, args):
        ...
        h_conv1 = Activation("relu")(
            BatchNormalization(axis=3)(Conv2D(args.num_channels, 3, padding="same")(x_image))
        )  # batch_size  x board_x x board_y x num_channels
        h_conv2 = Activation("relu")(
            BatchNormalization(axis=3)(Conv2D(args.num_channels, 3, padding="same")(h_conv1))
        )  # batch_size  x board_x x board_y x num_channels
        h_conv3 = Activation("relu")(
            BatchNormalization(axis=3)(Conv2D(args.num_channels, 3, padding="same")(h_conv2))
        )  # batch_size  x (board_x) x (board_y) x num_channels
        h_conv4 = Activation("relu")(
            BatchNormalization(axis=3)(Conv2D(args.num_channels, 3, padding="valid")(h_conv3))
