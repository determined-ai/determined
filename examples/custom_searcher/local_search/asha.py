...
ASHASearchMethod provides implementation for abstract methods in SearchMethod.
Those methods are invoked by LocalSearchRunner in response to the SearcherEvents
received from the remote experiment and produce Operations that are sent to the remote experiment.
For details about supported operation see:

To support fault tolerance, ASHASearchMethod implements save_state() and load_state() methods.
Those methods are responsible for saving and loading any information necessary for ASHASearchMethod
to resume in the case of failure. While implementation of save_state() and load_state() depends on the user,
...
ASHASearchMethodState object wth pickle.
'''
...
class ASHASearchMethod(SearchMethod):
    ...
    # information about trial progress and no new operations are created.
    ...
    # and based on current number of trials and metric value,
    ...
    ############################################################################
    # Invoked when a trial with specific request_id is completed.
    def on_trial_closed(self, request_id: uuid.UUID) -> List[Operation]:
        ...
        # Send Shutdown operation to close the experiment
        # if all requested trials are completed and
        # the number of maximum trials has been reached
        if (
    ...
    ############################################################################
    # User-defined function that generates hyperparameters that are passed to
    # the trial as a part of the Create operation.
    # TODO
