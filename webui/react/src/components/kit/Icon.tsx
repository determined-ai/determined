...
const ColumnsIcon: React.FC<{ color: string }> = ({ color }) => (
...
const OptionsIcon: React.FC<{ color: string }> = ({ color }) => (
  <svg fill="none" height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
    <path
      clipRule="evenodd"
...
const PanelIcon: React.FC<{ color: string }> = ({ color }) => (
  <svg fill="none" height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
    <path
      clipRule="evenodd"
...
const RowIconLarge: React.FC<{ color: string }> = ({ color }) => (
  <svg fill="none" height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
    <path
      clipRule="evenodd"
...
const RowIconMedium: React.FC<{ color: string }> = ({ color }) => (
  <svg fill="none" height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
    <path
      clipRule="evenodd"
...
const RowIconExtraLarge: React.FC<{ color: string }> = ({ color }) => (
  <svg fill="none" height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
    <rect height="13" rx="0.5" stroke={color} width="15" x="4.5" y="5.5" />
  </svg>
);

const RowIconSmall: React.FC<{ color: string }> = ({ color }) => (
  <svg fill="none" height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
    <path
      clipRule="evenodd"
...
const Icon: React.FC<Props> = ({
  ...
  const svgColor = React.useMemo(
    () => getCssVar(darkLight === DarkLight.Dark ? Status.InactiveStrong : Status.Inactive),
    [darkLight],
  );
  const rowIcon = React.useMemo(() => {
    if (size === 'small') return <RowIconSmall color={svgColor} />;
    if (size === 'large') return <RowIconLarge color={svgColor} />;
    if (size === 'big') return <RowIconExtraLarge color={svgColor} />;

    return <RowIconMedium color={svgColor} />; // returns medium by default
  }, [size, svgColor]);
  const svgIcon = React.useMemo(() => {
    if (name === 'columns') return <ColumnsIcon color={svgColor} />;
    if (name === 'filter') return <FilterIcon color={svgColor} />;
    if (name === 'options') return <OptionsIcon color={svgColor} />;
    if (name === 'panel') return <PanelIcon color={svgColor} />;
    if (name === 'row') return rowIcon;

    return null;
  }, [svgColor, rowIcon, name]);
