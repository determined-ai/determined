  ...
  const handleSaveDisplayName = useCallback(
    async (newValue: string): Promise<void | Error> => {
      try {
        ...
        makeToast({ containerRef, severity: 'Confirm', title: API_DISPLAYNAME_SUCCESS_MESSAGE });
      } catch (e) {
        handleError(containerRef, e, { silent: false, type: ErrorType.Input });
        return e as Error;
      }
    },
    [currentUser?.id],
  ...
  const handleSaveUsername = useCallback(
    async (newValue: string): Promise<void | Error> => {
      try {
        ...
        makeToast({ containerRef, severity: 'Confirm', title: API_USERNAME_SUCCESS_MESSAGE });
      } catch (e) {
        makeToast({ containerRef, severity: 'Error', title: API_USERNAME_ERROR_MESSAGE });
                ...
                <div className={css.section}>
                  <InlineForm<Mode>
                    ...
                    <Select searchable={false}>
                      ...
                      <Option
                        key={ThemeOptions.light.className}
                        value={ThemeOptions.light.className}>
