import { getColorScale, getNumericRange, updateRange, normalizeRange, distance, defaultNumericRange } from './chart';

/* describe('getColorScale', () => {
    it('No input', () => {
        expect(getColorScale()).toStrictEqual({color: string, scale: number})
    });
}); */

describe('defaultNumericRange', () => {
    it('Default', () => {
        expect(defaultNumericRange()).toStrictEqual([Number.NEGATIVE_INFINITY, Number.POSITIVE_INFINITY])
    })
    it('Reverse', () => {
        expect(defaultNumericRange(true)).toStrictEqual([ Number.POSITIVE_INFINITY, Number.NEGATIVE_INFINITY])
    })
})

describe('getNumericRange', () => {
    it('No values', () => {
        expect(getNumericRange([])).toStrictEqual(undefined)
    });
    it('Get range', () => {
