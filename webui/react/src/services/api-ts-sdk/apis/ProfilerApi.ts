...
export class ProfilerApi extends runtime.BaseAPI {
     ...
     * Stream the available series in a trial\'s profiler metrics.
     */
    async getTrialProfilerAvailableSeries(trialId: number, follow?: boolean, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<StreamResultOfV1GetTrialProfilerAvailableSeriesResponse> {
        const response = await this.getTrialProfilerAvailableSeriesRaw({ trialId: trialId, follow: follow }, initOverrides);
     ...
     * Stream trial profiler metrics.
     */
    async getTrialProfilerMetrics(labelsTrialId: number, labelsName?: string, labelsAgentId?: string, labelsGpuUuid?: string, labelsMetricType?: GetTrialProfilerMetricsLabelsMetricTypeEnum, follow?: boolean, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<StreamResultOfV1GetTrialProfilerMetricsResponse> {
        const response = await this.getTrialProfilerMetricsRaw({ labelsTrialId: labelsTrialId, labelsName: labelsName, labelsAgentId: labelsAgentId, labelsGpuUuid: labelsGpuUuid, labelsMetricType: labelsMetricType, follow: follow }, initOverrides);
        return await response.value();
    }
 ...
 * @export
 */
export const GetTrialProfilerMetricsLabelsMetricTypeEnum = {
...
export type GetTrialProfilerMetricsLabelsMetricTypeEnum = typeof GetTrialProfilerMetricsLabelsMetricTypeEnum[keyof typeof GetTrialProfilerMetricsLabelsMetricTypeEnum];
