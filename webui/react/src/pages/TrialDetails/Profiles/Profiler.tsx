...
export const CHART_STYLE: React.CSSProperties = { height: '100%', paddingBottom: 36 };
...
export const getSeriesForMetricName = (name: string, index: number): uPlot.Series => ({
  label: name,
  points: { show: false },
  scale: 'y',
  spanGaps: true,
...
const Profiler: React.FC<Props> = ({ trial }) => {
  ...
  const getOptionsForMetrics = useCallback(
    (activeMetricName: string, metricNames: string[]): Partial<Options> => {
      return {
        axes: [ baseAxes.time, getAxisForMetricName(activeMetricName) ],
        /**
        ...
        series: [
          baseSeries.time,
          baseSeries.batch,
          ...metricNames.slice(1).map(getSeriesForMetricName),
        ],
        tzDate,
      };
    },
