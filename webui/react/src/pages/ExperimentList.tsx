...
import { ArchiveFilter, CommandTask, ExperimentItem, RunState } from 'types';
...
const ExperimentList: React.FC = () => {
  ...
  const handleLabelFilterApply = useCallback((labels: string[]) => {
    setSelectedRowKeys([]);
    updateSettings({ label: labels.length !== 0 ? labels : undefined });
  }, [ updateSettings ]);

  const handleLabelFilterReset = useCallback(() => {
    setSelectedRowKeys([]);
    updateSettings({ label: undefined });
  }, [ updateSettings ]);
  ...
  const handleStateFilterApply = useCallback((states: string[]) => {
    setSelectedRowKeys([]);
    updateSettings({ state: states.length !== 0 ? states as RunState[] : undefined });
  }, [ updateSettings ]);
