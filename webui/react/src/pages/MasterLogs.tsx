...
const ANSI = `\x1b[31mhello\x1b[41;30mworld\x1b[m
Standard colors:
...
const MasterLogs: React.FC = () => {
  const [ logs, setParams ] = useRestApiSimple<LogsApiParams, Log[]>(getMasterLogs, DEFAULT_PARAMS);

  const ansiLogs = useMemo(() => {
    const ansi = {
      id: 999,
      message: ANSI,
      time: '2020-05-19T18:02:40.242697-06:00',
    };
    return [ ansi, ...(logs.data || []) ];
  }, [ logs.data ]);
  // const fetchLogs = useCallback(async (): Promise<void> => {
  //   requestLogs({ tail: 10000 });
  // }, [ requestLogs ]);
  ...
  return (
    <div className={css.base}>
      <Section maxHeight title="Master Logs">
        <LogViewer data={ansiLogs} fullPage />
      </Section>
    </div>
  );
};
