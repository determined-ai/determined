...
def _upload_migration_length(conn) -> None:
    authentication.cli_auth = authentication.Authentication(
        "http://127.0.0.1:8080", requested_user="admin", password="",
    )
    ...
    for l in bindings.get_MasterLogs(sess):
        if "running DB migrations from" in l.logEntry.message:
            migration_start_log = l.logEntry
            continue

        match = re.search(r"migrated from (\d+) to (\d+)", l.logEntry.message)
        if match:
            from_version = int(match.group(1))
            to_version = int(match.group(2))
            migration_end_log = l.logEntry
            continue

        if "no migrations to apply" in l.logEntry.message:
            no_migration_to_apply_log = l.logEntry
    ...
    print(f"migration start message '{migration_start_log.message}' at {migration_start_log.timestamp}")
    ...
    try:
        commit = subprocess.check_output(['git', 'log', '-1', '--pretty=format:%H'], universal_newlines=True).strip()
