...
optimizer is set to :mod:`automatically handle zeroing out the gradients
<determined.pytorch.PyTorchTrialContext.step_optimizer>`, ``step_optimizer`` will zero out the
gradients and there will be no need to call ``optim.zero_grad()``.
