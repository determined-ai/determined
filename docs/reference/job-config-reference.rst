...
-  ``environment``: Specifies the environment of the container that is used to execute the task.
   ...
   -  ``image``: The Docker image to use when executing the workload. This image must be accessible
      ...
      -  ``determinedai/environments-dev:cuda-11.3-pytorch-1.12-tf-2.11-gpu-0.26.5`` for NVIDIA GPUs.
      -  ``determinedai/environments-dev:rocm-5.0-pytorch-1.10-tf-2.7-rocm-0.26.5`` for ROCm.
      -  ``determinedai/environments-dev:py-3.8-pytorch-1.12-tf-2.11-cpu-0.26.5`` for CPUs.
