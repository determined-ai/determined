...
For general information about using Determined with Kubernetes, refer the
...
-  The Kubernetes cluster should be running Kubernetes versoin >= 1.19 and <= 1.21, though later
   versions may work.
...
-  The nodes in the cluster already have or can pull the ``fluent/fluent-bit:1.6`` Docker image from
   Docker Hub.
...
example, 8-core node may provide 7.91 CPUs, with the rest allocated for the Kubernetes system tasks.
If ``slotResourceRequests.cpu`` was set to 8 in this example, the pods would fail to allocate, so it
should be set to a lower number instead, such as 7.5.
...
#. **TLS termination in the Determined master.** This option will provide TLS encrytption inside the
   Kubernetes cluster. All communication with the master will encrypted. Communication between task
   containers (distributed training) will not be encrypted. To configure this option create a
