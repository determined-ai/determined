...
When installing :ref:`Determined on Kubernetes <install-on-kubernetes>` via Helm, the deployment
...
-  ``taskContainerDefaults``: Specifies Docker defaults for all task containers. A task represents a
   ...
   -  ``gpuImage``: Sets the default docker image for all gpu tasks. If a docker image is specified
      ...
      to: ``determinedai/environments:cuda-11.3-pytorch-1.10-lightning-1.5-tf-2.8-gpu-0.17.15``.
...
-  ``defaultScheduler``: Configures the default scheduler that Determined will use. Currently
   ...
   as ``coscheduler``, Determined will use the default Kubernetes scheduler. See
   :ref:`scheduling-on-kubernetes` for more details.
