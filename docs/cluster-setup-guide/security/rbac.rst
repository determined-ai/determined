...
notebooks, tensorboards, sheels, and commands (NTSC) are global entities. By default, every logged
in user can see all NTSC on the cluster. In the future, we plan to migrate NTSCs to workspaces to
make them covered under RBAC model. As an interim solution, we offer an option which restricts the
access to NTSC only to the user who created the task, i.e. its owner. It can be enabled in the
master config:
