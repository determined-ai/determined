...
``https_proxy`` field in the :ref:`agent configuration <agent-configuration>`.

The custom image and credentials can be set as the defaults for all tasks launched in Determined,
using the ``image`` and ``registry_auth`` fields in the :ref:`master-configuration`. Make sure to
