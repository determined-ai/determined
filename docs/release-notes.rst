...
-  ``records_per_epoch`` has been dropped from PyTorch code paths. We were previously using this
...
-  Trial API: the unused ``latest_training`` field has been removed from ``GetTrial`` and
   ``GetExperimentTrials`` APIs due to slow performance.

**Bug Fixes**

-  CLI: Fix an issue where ``det user change-password`` will return an authentication error when
   trying to change current user's password.
...
-  PyTorchTrial and DeepSpeedTrial APIs: Deprecate ``TorchWriter`` and add a PyTorch
