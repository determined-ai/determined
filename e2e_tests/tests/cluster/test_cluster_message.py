...
def test_cluster_message_when_admin() -> None:
	...
	detproc.check_call(
		admin, ["det", "master", "cluster-message", "clear"],
	)
	resp = detproc.check_json(
		admin, ["det", "master", "cluster-message", "get", "--json"]
	)
	assert resp['clusterMessage'] is None, "there should not be a cluster message set after clearing"

	# Python's ISO format isn't RFC 3339, so it doesn't include the Z; add it
	end_time = (datetime.utcnow() + timedelta(days=500)).isoformat() + "Z"
	test_message = "test message 1"

	# Set the message
	detproc.check_call(
		admin, ["det", "master", "cluster-message",
			"set", "-m", test_message, "--end", end_time],
	)

	# Make sure master info has an active cluster message
	master_info = detproc.check_json(
		admin, ["det", "master", "info", "--json"],
	)
	assert master_info['clusterMessage'] is not None, "active cluster message should be visible in master info"

	# Check the result
	actual_message = detproc.check_json(
		admin, ["det", "master", "cluster-message", "get", "--json"],
	)['clusterMessage']
	assert actual_message['message'] == test_message, "the message returned by the API was not the one expected"
	assert actual_message['endTime'] == end_time, "the end time returned by the API was not what was expected"
	...
	detproc.check_call(
		admin, ["det", "master", "cluster-message",	"set",
		"-m", test_message, "--start", start_time, "--duration", duration]
	)

	# Make sure master info does not have an active cluster message since it's scheduled in the future
	master_info = detproc.check_json(
		admin, ["det", "master", "info", "--json"],
	)
	assert master_info['clusterMessage'] == None, "cluster message scheduled in the future should not be visible in master info"

	# Make sure cluster message *is* present in result of cluster-message get
	msg = detproc.check_json(
		admin, ["det", "master", "cluster-message", "get", "--json"]
	)['clusterMessage']
	assert msg['message'] == test_message, "cluster message returned by the API was not the one expected"
	assert msg['endTime'] == expected_end, "cluster message end time returned by the API was not the one expected"
	...
	assert master_info['clusterMessage'] == None, "cluster message should not be visible in master info after clearing"
...
def test_cluster_message_requires_admin() -> None:
	...
	assert master_info['clusterMessage'] is not None, "active cluster messages should be visible to non-admins"
