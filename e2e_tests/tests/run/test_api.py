...
def test_run_delete() -> None:
    sess = api_utils.user_session()
    exp_id = exp.create_experiment(
        sess, conf.fixtures_path("no_op/empty_model_dir.yaml"), conf.fixtures_path("no_op")
    )
    ...
    # validate response
    assert len(deleteResp.results) == 1
    assert deleteResp.results[0].id == run_id
    assert deleteResp.results[0].error == ""

    # ensure that run is deleted
    searchResp = bindings.get_SearchRuns(
        sess,
        limit=1,
        filter="""{"filterGroup":{"children":[{"columnName":"experimentId","kind":"field",
...
@pytest.mark.e2e_cpu
def test_run_delete_filter() -> None:
    sess = api_utils.user_session()
    exp_id = exp.create_experiment(
        sess,
        conf.fixtures_path("mnist_pytorch/adaptive_short.yaml"),
