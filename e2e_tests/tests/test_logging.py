...
@pytest.mark.parametrize(  # type: ignore
    ...
    [
        ...
        (command.TaskTypeTensorboard, {}, {"experiment_ids": [1]}, re.compile("^.*TensorBoard .* at .*$")),
    ],
)
def test_task_logs(task_type, task_config, task_extras, log_regex) -> None:
    # TODO: refactor tests to not use cli singleton auth.
    master_url = conf.make_master_url()
    certs.cli_cert = certs.default_load(conf.make_master_url())
    authentication.cli_auth = authentication.Authentication(conf.make_master_url(), try_reauth=True)
    ...
    if task_type == command.TaskTypeTensorboard and \
        not api.get(master_url, "/api/v1/experiments").json()['experiments']:
...
def check_logs(master_url, id, log_regex, log_fn, log_fields_fn):
    # This is also testing that follow terminates. If we timeout here, that's it.
    match = False
    for log in log_fn(master_url, id, follow=True):
    ...
    assert any(log_fn(master_url, id, tail=10)), "tail returned no logs"
    assert any(log_fn(master_url, id, head=10)), "head returned no logs"

    # Task log fields should work, follow or no follow.
    assert any(log_fields_fn(master_url, id, follow=True))
    fields = list(log_fields_fn(master_url, id))
    assert any(fields), "no task log fields were returned"
    ...
    for k, v in fields.items():
        ...
        assert any(log_fn(master_url, id, **{
            to_snake_case(k): v[0],
        })), "good filter returned no logs"

    # Check nonsense is nonsense.
    assert not any(log_fn(master_url, id, rank_ids=[-1])), "bad filter returned logs"
