...
class SimulateMaster(): 
    ...
    def _get_event_for_op(self,op: Operation):
        if type(op) == ValidateAfter: 
            ...
            event = bindings.v1SearcherEvent.validationCompleted(requestId=str(op.request_id), metric=metric, validateAfterLength=str(op.length))
        
        if type(op) == Create: 
              event = bindings.v1SearcherEvent.trialCreated(requestId=str(op.request_id))
    ...
    def random_validation(trial_id) -> float64: 
        import random 
        return random.random() 
    
    def trial_id_metric(trial_id) -> float64:
        return float64(trial_id)
...
class FallibleSearchRunner(LocalSearchRunner):
    ...
    def post_operations(
        ...
        # send these operations where you can create the specific events. Each operations has a corresponding event. You just need to create those here. And return those events. 
        events = self.mock_master_obj.process_operations(experiment_id, event, operations)