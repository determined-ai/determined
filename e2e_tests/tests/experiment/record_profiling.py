    ...
    def record(self, trial_id: int):
        for metric in self.metrics:
            ...
            for method in self.metrics[metric]:
                metric_key, metric_value = format_xml_property(metric,
                                                               method,
                                                               summary_methods[method](metrics))
                self.record_property(metric_key, metric_value)


def format_xml_property(metric_type: str, summary_method: str,
...
def get_profiling_metrics(trial_id: int, metric_type: str) -> List[float]:
    ...
    with api.get(
        ...
        return [batch for batches in
                [simplejson.loads(line)["result"]["batch"]["values"]
