...
DefType = typing.Union[swagger_types.TypeDef, swagger_types.Function]
...
def load(anno: SwaggerType, val: Code) -> Code:
    ...
    if isinstance(anno, swagger_types.Ref):
        assert anno.defn
        assert isinstance(anno.defn, (swagger_types.Enum, swagger_types.Class)), (
            anno.name,
            type(anno.defn).__name__,
        )
...
def dump(anno: SwaggerType, val: Code, omit_unset: Code) -> Code:
    ...
    if isinstance(anno, swagger_types.Parameter):
        return dump(anno.type, anno.name, omit_unset)
    if isinstance(anno, swagger_types.Class):
...
def gen_def(anno: DefType) -> Code:
    ...
    if isinstance(anno, swagger_types.Function):
        # Function name.
        ...
        if "body" in anno.params:
            ...
            bodystr = dump(anno.params["body"], "", "True")
        else:
            bodystr = "None"
